...
const mockDatabase = [
  {
    ...
    urlSlug: "/img-world-tickets/img-worlds-of-adventure-tickets/",
...
const AlgoliaLikeSearch = () => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto p-4" ref={searchRef}>
        {/* <Header3 /> */}
        <div className="relative bg-white rounded-full shadow-lg overflow-visible">
        <div className="absolute left-6 top-1/2 transform -translate-y-1/2 text-gray-500 pointer-events-none">
          {!isSearchFocused && !searchTerm && "Search for"}
        </div>
        <input
          type="text"
          className="w-full pl-28 pr-16 py-4 bg-transparent focus:outline-none rounded-full"
          placeholder={
            isSearchFocused || searchTerm ? "" : "attractions, tours, etc."
          }
          value={searchTerm}
