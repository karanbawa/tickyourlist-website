    ...
    <div className={`AvatarDropdown relative flex ${className}`}>
      {user ? (
        <Popover className="flex items-center">
          {({ open, close }) => (
            <>
              ...
              >
                <Avatar sizeClass="w-8 h-8 sm:w-9 sm:h-9" imgUrl={user?.imageUrl ?? ''} />
              </Popover.Button>
              <Transition
                as={Fragment}
                  ...
                  <div className="overflow-hidden rounded-3xl shadow-lg ring-1 ring-black ring-opacity-5">
                    <div className="relative grid grid-cols-1 gap-6 bg-white dark:bg-neutral-800 py-7 px-6">
                      <div className="flex items-center space-x-3">
                        <Avatar sizeClass="w-12 h-12" imgUrl={user?.imageUrl ?? ''} />

                        <div className="flex-grow">
                          <h4 className="font-semibold">{user.name}</h4>
                          <p className="text-xs mt-0.5">{user.location}</p>
                        </div>
                      </div>
        ...
        <div className="flex items-center space-x-4">
          <Link href="/login">
            <p className="text-sm font-medium text-neutral-700 dark:text-neutral-300 cursor-pointer">Sign In</p>
          </Link>
        </div>
      )}
    </div>
